...
def sort_crops():
    try:
        ...
        sorted_result, metrics = selection_sort_with_metrics(crops_query, sort_by, is_descending=is_descending)
...
def selection_sort_with_metrics(query_result, key, is_descending=False):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            if (compare(arr[j][key], arr[extreme_idx][key]) < 0) != is_descending:
                extreme_idx = j
        # Swap the found extreme element with the current element
